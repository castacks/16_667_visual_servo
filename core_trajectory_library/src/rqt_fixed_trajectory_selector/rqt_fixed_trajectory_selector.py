...
class FixedTrajectorySelectorPlugin(Plugin):
    def __init__(self, context):
        ...
        # self.yawrate_switch_pub = rospy.Publisher('wrench_controller/switchYawRate',Bool,queue_size=1)
        # self.Z_switch_pub = rospy.Publisher('wrench_controller/switchZ',Bool,queue_size=1)
        # self.Y_switch_pub = rospy.Publisher('wrench_controller/switchY',Bool,queue_size=1)
        ...
        self.publish_button = qt.QPushButton('Save Target')
    ...
    def init_buttons(self, filename):
        ...
        def control_mode_publish(topic, value):
            def button_pressed():
                if topic not in self.control_mode_pub:
                    self.control_mode_pub[topic]=rospy.Publisher('visual_servo_switch/'+topic,UInt8,queue_size=1)
                msg = UInt8()
                msg.data = value
                self.control_mode_pub[topic].publish(msg)
                rospy.loginfo("send switch signal: Topic: %s  Value: %s"%('wrench_controller/'+topic,value))
        ...
        for trajectory in y['trajectories']:
            trajectory_name = next(iter(trajectory.keys()))
            ...
            try:
                control_modes = trajectory[trajectory_name]['control_mode']
            except:
                control_modes = {}
            ...
            for topic, options in control_modes.items():
                ...
                for key,value in options.items():
                    button_edit = qt.QRadioButton(key)
                    button_edit.pressed.connect(control_mode_publish(topic, value))
                    if value==0:
                        button_edit.setChecked(True)
                    button_layout.addWidget(button_edit)
                    button_group.addButton(button_edit)
                
                
                trajectory_layout.addWidget(button_widget)
